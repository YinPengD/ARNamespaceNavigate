...
- (void)setUI
{
    ...
    self.isFirst = YES;  // 第一次进入时显示公交站台的信息
...
- (instancetype)addSearchViewController:(UIViewController *)vc{
    //初始化视图，添加搜索面板
    [self.shadowView addSubview:self.searchVC.view];
    // 添加搜索控制
    [vc addChildViewController:self.searchVC];
    //把视图添加到面板中
    [vc.view addSubview:self.shadowView];
    // 点击事件的调用
    [self.searchVC didClickTextField:^{
...
// table可滑动时，swipe默认不再响应 所以要打开，设置的下层滑动视图
