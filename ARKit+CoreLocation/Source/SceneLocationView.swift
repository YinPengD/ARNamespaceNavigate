...
public class SceneLocationView: ARSCNView, ARSCNViewDelegate {
    ...
    ///The method to use for determining locations.确定位置的方法
    ...
    public func run() {
        ...
        // Create a session configuration创建世界坐标追踪
    ...
    /// 当pointOfView有值时返回一个SCNVector3坐标,否则返回0
    ///
    /// - Returns:
    ...
    public func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        // 当图像的名字符合时
        if referenceImage.name == "lupai" {
            // 提取图像
            let w = referenceImage.physicalSize.width
            let h = referenceImage.physicalSize.height
            let plane1 = SCNPlane(width: w + 0.7, height: h)
            let temNode = SCNNode(geometry: plane1)
            let material = SCNMaterial()
            material.diffuse.contents = UIImage(named: "jiantou5")
            plane1.materials = [material]
            temNode.position = SCNVector3Make(1.2, 0, 0)
            temNode.eulerAngles = SCNVector3Make(-.pi/2.0, 0, 0)
            temNode.opacity = 0.8
            print("捕获")
            //将节点添加到跟节点上
            node.addChildNode(temNode);
            
            /******************************************************/
            DispatchQueue.main.async {
                if SceneLocationView.j < 2 {
                let filePath = Bundle.main.path(forResource: "0002", ofType: "mp4")
                let videoURL = URL(fileURLWithPath: filePath!)
                    let planeNode = SCNNode()
                    let plane = SCNPlane(width: w + 0.7, height: h + 0.3)
                    planeNode.geometry = plane
                    planeNode.geometry?.firstMaterial?.isDoubleSided = false
                    planeNode.position = SCNVector3(0, 0, 0.7)
                    planeNode.eulerAngles = SCNVector3Make(-.pi/2.0, 0, 0)
                    node.addChildNode(planeNode);
                    
                    let videoNode = SKVideoNode.init(url: videoURL)
                    ...
                    videoNode.play()
                    SceneLocationView.j = SceneLocationView.j + 1
            }
